...
export class WindowStateManager {
    ...
    async fetchGuestWindows(apiPort: number): Promise<GuestWindowInfo[]> {
        try {
            const response = await fetch(`http://localhost:${apiPort}/windows`, {
                signal: AbortSignal.timeout(3000),
            });

            if (response.status !== 200) {
                console.error("[WindowStateManager] Failed to fetch guest windows (HTTP status non-200):", response);
                return [];
            }
    ...
    private startMonitoring(): void {
        ...
        this.monitoringInterval = setInterval(async () => {
            for (const [wmClass, state] of this.states.entries()) {
                ...
                if (state.connectionState === "disconnected" && Date.now() - state.lastSeen > 5 * 60 * 1000) {
    ...
    getDisconnectedWindows(): WindowState[] {
        return Array.from(this.states.values()).filter(
            s => s.connectionState === "disconnected" && s.reconnectAttempts < s.maxReconnectAttempts,
        );
    }
    ...
    attemptReconnect(wmClass: string, newFreerdpPid: number): void {
        ...
        console.log(
            `[WindowStateManager] Reconnect attempt ${state.reconnectAttempts}/${state.maxReconnectAttempts} for ${wmClass}`,
        );
        this.scheduleSave();
    }
    ...
    private async saveStates(): Promise<void> {
        try {
            const data = JSON.stringify(Array.from(this.states.entries()), null, 2);
